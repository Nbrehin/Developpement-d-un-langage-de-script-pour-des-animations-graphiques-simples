...
class NewElement implements Command {
	...
	public Reference run(Reference reference, SNode method) {
		...
		try {
			...
			Reference ref = new Reference(e, method.get(0).contents());
...
class NewImage implements Command {
	public Reference run(Reference reference, SNode method) {
		try {
			...
			Reference ref = new Reference(i, method.get(0).contents());
			ref.addCommand("translate", new Translate());
			ref.addCommand("setDim", new SetDim());
			return ref;
		} catch (IOException e) {
...
class NewString implements Command {
	public Reference run(Reference reference, SNode method) {
		...
		Reference ref = new Reference(s, method.get(0).contents());
		ref.addCommand("translate", new Translate());
		ref.addCommand("setColor", new SetColor());
		return ref;
	}
		...
		try {
			...
			if (reception instanceof String) {
				...
				if (MBLOC.mess.equalsIgnoreCase("execution")) {
					...
					GSpace space = (GSpace) this.environment.getReferenceByName("space").getReceiver();
					String envoie = spaceToImageBase64(space);
					String RBLOC = Message.toJson(new Message("mode", "IMG:" + envoie));
					...
					this.output.writeObject(RBLOC); // Envoi de l'image
				}
			}
		...
		while (itor.hasNext()) {
			...
			if (reception instanceof String) {
				...
				if (MPAS.mess.toLowerCase().equals("execution")) {
					...
					String envoie = spaceToImageBase64(space);
					try {
						String RPASI = Message.toJson(new Message("mode", "IMG:" + envoie));
						String environmentTree = environment.environmentToArbre();
						this.environment.printed.clear();
						String environmentMessage = Message.toJson(new Message("Environment", environmentTree));
						this.output.writeObject(environmentMessage);
