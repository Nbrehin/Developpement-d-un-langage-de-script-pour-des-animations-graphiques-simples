...
import Controller5.UIController;
...
public class Robi_client implements Client {
	...
	public void decode(String cmd) {
		String[] champs = cmd.split(" ");
		this.elements.put(champs[6], new Element(Double.parseDouble(champs[1]), Double.parseDouble(champs[2]), Double.parseDouble(champs[3]), Double.parseDouble(champs[4]),champs[0],champs[5],champs[6]));
		javafx.application.Platform.runLater(() -> {
			ui.canvasView.getGraphicsContext2D().clearRect(0, 0, 200, 200);
			ui.canvasView.getGraphicsContext2D().setFill(Color.WHITE);
			ui.canvasView.getGraphicsContext2D().fillRect(0, 0, 200, 200);
		});
		this.elements.forEach((key, value) -> {
			System.out.println(key);
			if(value.type.equals("rect.class")) {
				Paint p = Paint.valueOf(value.getCouleur());
				javafx.application.Platform.runLater(() -> {
					
					ui.canvasView.getGraphicsContext2D().setFill(p);
					ui.canvasView.getGraphicsContext2D().fillRect(value.getX(), value.getY(),value.getW(), value.getH());
				});
			}else {
				if(value.type.equals("oval.class")) {
					Paint p = Paint.valueOf(value.getCouleur());
					javafx.application.Platform.runLater(() -> {
						ui.canvasView.getGraphicsContext2D().setFill(p);
						ui.canvasView.getGraphicsContext2D().fillOval(value.getX(), value.getY(),value.getW(), value.getH());
					});
				}
			}
		});
					...
					try {
						if (input != null) {
							...
							if (received instanceof String) {
								...
								} else if (response.mess.startsWith("CMD:")) {
